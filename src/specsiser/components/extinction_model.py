...
class ExtinctionModel:
    ...
    def cHbeta_from_log(self, line_df, line_labels='all', temp=10000.0, den=100.0, ref_wave='H1_4861A',
                        comp_mode='auto', plot_address=False, input_flux_labels=('obsFlux', 'obsFluxErr')):
        ...
        if len(line_labels) > 1:
            ...
            # Observed ratios
            if comp_mode == 'auto':
                if ('obsFlux' in line_df.columns) and ('obsFluxErr' in line_df.columns):
                    obsFlux = line_df.loc[idcs_lines, input_flux_labels[0]].values
                    obsErr = line_df.loc[idcs_lines, input_flux_labels[1]].values
                    Href_flux, Href_err = line_df.loc[ref_wave, 'obsFlux'], line_df.loc[ref_wave, 'obsFluxErr']
                else:
            ...
            cHbeta, cHbeta_err = np.nan, np.nan
